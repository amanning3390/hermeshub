...
# Fetch from ESPN (NBA only)
fetch_espn_nba() {
  local url="https://site.api.espn.com/apis/site/v2/sports/basketball/nba/scoreboard"
  curl -sf --max-time 15 -H "User-Agent: ${UA}" "$url" 2>/dev/null || echo '{"events":[]}'
}
...
for e in events:
    ...
    if len(comps) < 2:
        continue
    # ESPN: index 0 is home (sometimes), check homeAway field
    ...
    print(f'NBA|{h_name}|{a_name}|{h_score}|{a_score}|{status}')
" 2>/dev/null
}
...
case "$SPORT" in
  ...
  Basketball)
    ...
    # Fallback to ESPN if TheSportsDB returned nothing
    if [[ -z "$RESULT" ]]; then
      RESULT=$(fetch_espn_nba | parse_espn | apply_filter) || true
    fi
    ;;
  Soccer|*)
